...
const Reveal = () => {
  const [inputs, initialize, setEventId] = useFinishGameStore((state) => [state.inputsRevealAnswer, state.initialize, state.setEventId])
  const [currentGame] = useGameStore((state) => [state.currentGame, state.puzzleRecords]);
  ...
  useEffect(() => {
    if (!currentGame || !msPuzzleRecords || !msGameRecords || !currentGame.utilRecords)
      return;
    const reveal_answer_notification_record = currentGame.gameNotification.recordWithPlaintext;
    ...
    const joint_piece_stake = currentGame.puzzleRecords.find(
      (r) =>
        r.data.ix.replace('.private', '') === '10u32'
    );

    const challenger_claim_signature = currentGame.puzzleRecords.find(
      (r) =>
        r.data.ix.replace('.private', '') === '7u32'
    );

    console.log('reveal_answer_notification_record', reveal_answer_notification_record);
    ...
    if (!reveal_answer_notification_record || !challenger_answer_record || !joint_piece_stake || !challenger_claim_signature) return;

    initialize(reveal_answer_notification_record, challenger_answer_record, joint_piece_stake, challenger_claim_signature)
  }, [
    currentGame?.gameNotification.recordData.game_multisig,
    [msPuzzleRecords, msGameRecords, currentGame?.utilRecords, currentGame?.puzzleRecords].toString(),
  ])
  ...
  const answer = inputs?.challenger_answer_record?.data.answer === '0field.private' ? Answer.InTheWeeds : Answer.BehindTheBuilding;
  ...
  return (
    <div className='flex h-full w-full flex-col gap-4'>
      ...
      <Button color='green' onClick={createEvent} disabled={disabled || loading}>
        REVEAL RESULTS
      </Button>
    </div>
  );
